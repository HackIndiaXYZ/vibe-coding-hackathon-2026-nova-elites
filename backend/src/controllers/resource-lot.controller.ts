...
import { createResourceLotSchema, updateResourceLotSchema } from '../schemas/resource.schema';
import { createResourceLot, getResourceLots, getLotById, updateResourceLot } from '../services/resource-lot.service';
...
export const updateResourceLotController = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const validatedData = updateResourceLotSchema.parse(req.body);
  
  // Get organizationId from the authenticated context (mocked via query param for now, matching getTransfers pattern)
  ...
  const updatedLot = await updateResourceLot(id, organizationId, validatedData);
  res.status(200).json(createSuccessResponse(updatedLot));
});
